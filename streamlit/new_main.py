...
url_options = {
    ...
    "Open AI - Gemini": "https://us-central1-kloudstax-429211.cloudfunctions.net/openai-gemini"
}
selected_url_name = st.selectbox("Select Cloud Function", list(url_options.keys()))
gcf_url = url_options[selected_url_name]

# Add a checkbox for the 'rank' parameter
rank = False
if selected_url_name == "Open AI - Gemini":
    rank = st.checkbox("rank the product", value=False)
...
if st.button("Send Request"):
    if user_query and bearer_token:
        ...
        # Build the request URL based on the selected Cloud Function
        if selected_url_name == "Open AI - Gemini":
            # Append the 'rank' parameter
            request_url = f"{gcf_url}?query={user_query}&rank={str(rank).lower()}"
        else:
            request_url = f"{gcf_url}?query={user_query}"
